...
def _run_job_start(
    ...
    * `-n`/`--dry-run` and `--timeout VALUE` (or `--timeout=VALUE`) are
      consumed wherever they appear, even interleaved with passthrough.
    * The first token not consumed by a known option names the TEMPLATE —
      including tokens after `--`.
    * The first `--` is consumed (not forwarded); every other token is
      passthrough to salloc, in its original order.
    """
    _require_sol()
    dry_run = False
    ...
    while i < len(tail):
        ...
        elif tok in ("-h", "--help") and not dd_seen and help_parser is not None:
...
def main(argv: list[str] | None = None) -> None:
    ...
    # `--version` among the root options and the `version` subcommand
    # short-circuit everything else: no Sol check, no subparser dispatch.
