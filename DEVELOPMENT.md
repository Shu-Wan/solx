...
| **L2 — Mocked Sol** | Real scripts (today: `sol_renew.py`; later: `solx`) executed against a fake Sol environment. Catches: parsing the warning CSVs, `.solkeep` matching, `uv` shebang bootstrap, side-detection logic. | Laptop or Sol login (no privileges needed — pure userland mocks) | Run script → assert on exit code + stdout/stderr + filesystem mutations + mock invocation log |
